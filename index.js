...
const { LifecycleScope, ReconciliationQueue, TreeState } = require('./lib/tree')

const DOT_RE = /\..*\.(sw[px])$|~$|\.subl.*\.tmp/
const WATCH_BACKENDS = new Set(['auto', 'native', 'native-recursive', 'polling'])
...
class FSWatcher extends EventEmitter {
  ...
  constructor(_opts = {}, scheduler = systemScheduler) {
    super()
    ...
    const envPoll = rt.env('CHOKIDAR_USEPOLLING') // rt: S8
    ...
    const envInterval = rt.env('CHOKIDAR_INTERVAL') // rt: S8
    ...
    this.lifecycle = new LifecycleScope(
      ...
      (error) => {
        if (!this.closed)
          this.handleError(error instanceof Error ? error : new Error(String(error))) // lunte-disable-line curly
      }
    )
    ...
    this.events = new EventPolicy({
      ...
      publish: (event, args) => this.emitWithAll(event, args)
    })
    void this.emitRaw
  ...
  isPathGenerationActive(path, generation) {
  ...
  close() {
    ...
    this.closePromise = (async () => {
      ...
      const failed = closerResults.find((result) => result.status === 'rejected')
   ...
   */
  async emitEvent(event, path, stats) {
   ...
   */
  removePath(directory, item, isDirectory) {
    // if what is being deleted is a directory, get that directory's paths
    // for recursive deleting and cleaning of watched object
    // if it is not a directory, nestedDirectoryChildren will be empty array
    const path = sp.join(directory, item)
    const logicalKey = logicalPathKey(path)
    isDirectory = isDirectory != null ? isDirectory : this.tree.watched.has(logicalKey) // lunte-disable-line eqeqeq
   ...
   */
  closePath(path, recursive = false) {
    const logicalKey = logicalPathKey(path)
  ...
  createScanStream(root, opts) {
    if (this.closed) return
    const options = { type: EV.ALL, alwaysStat: true, lstat: true, depth: 0, ...opts }
    const stream = readDir(root, options) // rt: S13
