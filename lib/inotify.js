...
// result of uv_fs_event_start(), so a watch armed past fs.inotify.max_user_watches is a handle that
...
// release reports uv_fs_event_start() failures, gate it off by version here.
...
function flush() {
  ...
  const { verified, failed } = reconcile(batch, before, after)
...
function take() {
  ...
  const max = readLimit()
  if (max && state.armed + 1 > max - RESERVE) return false
  if (needsVerification() && state.pending.length === 0 && state.before === null) {
...
function verify(path, onFail) {
  ...
  state.pending.push({ path, key, isNew, onFail })
...
function facts() {
  return { armed: state.armed, limit: readLimit(), reserve: RESERVE, verified: state.known.size }
}

// Test seam.
function _reset({ limit } = {}) {
