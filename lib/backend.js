...
const { watch: fsWatch, realpathSync } = require('fs') // rt: S1
...
function invalidateWatcherResource(resource, registry) {
  ...
  if (registry === FsWatchInstances) inotify.release() // DV11: per-directory handles are budgeted
  return subscribers
}
...
function toNativeWatchPath(path) {
  ...
  try {
    ...
    return realpathSync(nativePath) // rt: S12
  } catch {
    return nativePath
  }
}
...
function createFsWatchInstance(path, options, listener, errHandler) {
  try {
    ...
    if (!inotify.take()) throw inotifyBudgetExhausted(path) // DV11
    try {
...
function createNativeContainer(path, nativePath, resourceKey, options, subscriber) {
  ...
  // DV11: bare-fs never reports a failed uv_fs_event_start(); ask the kernel instead. A watch it
  // does not hold surfaces as ENOSPC through the same path as a native error.
  inotify.verify(nativePath, (error) => {
    void handleNativeError(path, cont, error)
  })
  return cont
}
