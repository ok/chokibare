...
function selectBackend(options) {
  ...
  // bare-fs cannot report that recursive is unsupported on Linux (libuv silently ignores the flag), so the runtime latch upstream relies on can never fire there.
  const kind =
    options.backend === 'polling'
      ? 'polling'
      : preferRecursive && options.depth === undefined && rt.platform !== 'linux' // DV11
